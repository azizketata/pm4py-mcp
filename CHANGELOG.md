...
**Advanced visualization (2 tools)** — PNG-only single-channel output via new `src/pm4py_mcp/_matplotlib.py::save_matplotlib_png` helper (parallel to `save_dual_channel`'s PNG+SVG). The underlying pm4py functions (`save_vis_dotted_chart`, `save_vis_performance_spectrum`) turned out to be **Graphviz/neato-backed under the hood** despite the matplotlib-flavoured naming, so both tools call `check_graphviz()` and raise `GraphvizMissing` with the same actionable error as the Phase 1 viz tools when the `dot` / `neato` binaries are absent. Argument validation runs *before* the Graphviz check so `UnsupportedFormat` for bad attrs/activities surfaces without requiring Graphviz.
